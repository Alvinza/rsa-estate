...
const Projects = () => {
    // State for tracking the current project index in the slider
    const [currentIndex, setCurrentIndex] = useState(0);
    // State for tracking the number of cards to show based on screen width
    const [cardsToShow, setCardsToShow] = useState(1);

    // Effect to handle updating the number of cards shown based on window size
    useEffect(() => {
        const updateCards = () => {
        ...
        updateCards(); // Initial call to set the correct number of cards

        window.addEventListener('resize', updateCards); // Update cards on window resize
        return () => window.removeEventListener('resize', updateCards); // Cleanup listener
    }, [])
    ...
    return (
        ...
        className='container mx-auto py-4 pt-20 px-6 md:px-20 lg:px-32 my-20 w-full overflow-hidden' id='Projects'>
            ...
            {/* Slider buttons for navigating projects */}
